...
export function PriceStep() {
  ...
  const shorthandPreview = propertyData.price && propertyData.price >= 1000
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="space-y-4">
        ...
        {/* Real-time magnitude confirmation (Lagos Luxury UX) */}
        <div className="h-8 flex items-center justify-center -mt-4" aria-live="polite">
          <AnimatePresence mode="popLayout">
            {shorthandPreview && (
              <motion.span
                key={shorthandPreview}
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -10 }}
                transition={{ duration: 0.3, ease: "easeOut" }}
                className="text-emerald-600 font-bold text-lg tracking-tight"
              >
                {shorthandPreview}
              </motion.span>
            )}
